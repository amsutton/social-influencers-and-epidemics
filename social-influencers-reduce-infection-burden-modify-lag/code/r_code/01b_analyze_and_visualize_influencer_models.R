#Build diagnostic plots of individual models
  ...
  #### VISUALIZE BASIC INFECTION CURVE COMPARISONS ####
  
  process_viz_infections = function(dat,string_one,string_two) {
    ...
    dat_for_peak = dat %>%
      ...
      mutate(behavior = "do behavior", #engage in behaviour
             status = ifelse(status ==  "I",
                             ...
                             "susceptible"),
             group = factor(ifelse(group == 1,
                                   paste0("early-infected group: ",
                                   g1_influencer_message_phrase),
                                   paste0("late-infected group: ", 
                                   g2_influencer_message_phrase)))) %>%
      ungroup() %>%
...
#
  ...
  if (nrow(step_at_the_peak) != 2000){
    ...
    temp = 
      ...
      mutate(n_status = 0,
             step_at_the_peak = NA,
             group = case_when(group == 'early-infected group: health-protective' ~ 'late-infected group: anti-protective',
                               group == 'early-infected group: anti-protective' ~ 'late-infected group: health-protective',
                               group == 'late-infected group: health-protective' ~ 'early-infected group: anti-protective',
                               group == 'late-infected group: anti-protective' ~ 'early-infected group: health-protective'))
  ...
  #### load model id list ####
  modellist = readRDS(here("data/model_id_list.RDS"))
  
  modellist = modellist %>% na.omit %>% distinct(string_one,string_two)
  ...
  modellist = modellist %>%
    ...
    filter(!str_detect(string_one,"aversion=0.1"), #these were run in Julia, but they are not included in this paper (results too similar to others shown)
           !str_detect(string_one,"aversion=0.3_f=3_g1_influencer_message=0_g2_influencer_message=1_")) 
  ...
  modellist = rbind(modellist[1,],modellist[4:21,])

  for (i in 1:nrow(modellist)){

      model_filenames <- intersect(list.files(
        path = here("data/clean_abm_results_noRstatus"), 
      ...
      #load the clean simulation data for the model parametrisation
      dat = readRDS(here(paste0(model_filenames)))

      string_one = modellist$string_one[i]
      string_two = modellist$string_two[i]

   process_viz_infections(dat,string_one,string_two)
